...
class RepositoryMemDataTest {
    ...
    fun `getAll Data entries`() {
        ...
        val dataList = listOf<Data>(
            ...
            Data(
                id = 2,
                odsId = listOf(5, 6, 7),
                type = DataType.ACAO_NA_SOCIEDADE,
                origin = "Origin1",
                dateChecked = fixedDate
            ),
    ...
    fun `getOds returns the list of ODS associated with the Data`() {
        val foundData = repo.getById(1)
        assertNotNull(foundData)
        val foundOds = repo.getOds(foundData.id)
        assertEquals(listOf(Ods(3, "Educação de Qualidade"), Ods(16, "Parcerias Emprol das Metas")), foundOds)
    }
